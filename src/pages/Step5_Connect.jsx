// src/pages/Step5_Connect.jsx — Enter voucher and connect to http://ntnafro.net/login
import { useState, useEffect } from 'react';
import { useNavigate, useLocation } from 'react-router-dom';
import { Wifi, CheckCircle2, ArrowRight, Signal, Clock, Gauge, Ticket, ExternalLink } from 'lucide-react';
import { StepShell, RedButton, GhostButton } from '../components/shared';
import { submitHotspotLogin, HOTSPOT_LOGIN_URL } from '../utils/store';
...
export default function Step5Connect() {
  ...
  const handleChange = (e) => {
    // Alphanumeric uppercase voucher (e.g. HVLA1Z7 or with hyphens)
    setVoucher(e.target.value.replace(/[^a-zA-Z0-9-]/g, '').toUpperCase());
    setStatus('idle');
  };
      ...
      <StepShell step={5} title="You're Connected! 🎉" subtitle="Enjoy ultra-fast internet at The Most Wanted Concert">
        <div className="text-center py-4">
          <div
            ...
            style={{
              background: 'linear-gradient(135deg, #E31E24 0%, #7c0f13 100%)',
          ...
          <div className="glass-card rounded-2xl p-4 mb-5 text-center">
            ...
            <div className="flex items-center justify-center gap-1.5 mt-2">
              {Array.from({ length: 5 }).map((_, i) => (
                <div
                  key={i}
                  className={`rounded-sm transition-all duration-300 ${i < bars ? 'bg-green-400' : 'bg-white/15'}`}
          ...
          <div className="grid grid-cols-3 gap-3 mb-6">
            {[
              { icon: Gauge, label: 'Speed', value: pkg?.speed || '15-20 Mbps', color: 'text-red-400' },
          ...
          <div className="glass-card rounded-2xl p-4 mb-6 text-left">
            ...
            <div className="space-y-2">
              {[
                ...
                ['Network SSID', 'WeAreAfro_NTN'],
                ['Hotspot Gateway', 'ntnafro.net'],
          ...
          <div className="space-y-3">
            <a
              href="http://ntnafro.net/status"
              target="_blank"
              rel="noopener noreferrer"
              className="btn-red w-full flex items-center justify-center gap-2 text-sm h-12"
            >
              Open Wi-Fi Status <ExternalLink className="w-4 h-4" />
            </a>
  ...
  return (
    <StepShell
      step={5}
      title="Connect & Enjoy!"
      subtitle="Enter your voucher code to log in to the hotspot"
    >
      {prefillVoucher && (
        <div
        ...
        >
          ...
          <span className="text-xs text-white/60">Voucher code pre-filled: {prefillVoucher}</span>
        </div>
      )}
      ...
      <div className="glass-card rounded-2xl p-5 mb-4">
        <div className="flex items-center justify-between mb-4 pb-3 border-b border-white/10">
          <span className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-1.5">
            <Ticket className="w-3.5 h-3.5 text-red-400" /> Hotspot Login (ntnafro.net)
          </span>
        ...
        <input
          ...
          className="input-field text-center text-2xl tracking-widest font-mono uppercase font-black mb-4"
          value={voucher}
        ...
        <p className="text-[11px] text-white/35 text-center mb-5">
          Submits voucher as username & password to <span className="font-mono text-white/60">http://ntnafro.net/login</span>
        </p>
        ...
        {status === 'connecting' ? (
          <div className="btn-red w-full h-14 flex items-center justify-center gap-3 text-base font-bold rounded-xl">
            ...
            Logging into Wi-Fi{dots}
          </div>
        ) : (
          <RedButton onClick={handleConnect} disabled={voucher.trim().length < 4}>
      ...
      <div className="text-center">
        <p className="text-xs text-white/30 mb-3">Don't have a voucher code?</p>
        <GhostButton onClick={() => navigate('/buy')}>
          Buy a Data Package via M-Pesa
        </GhostButton>
      </div>
    </StepShell>
  );
